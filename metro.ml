...
{kanji="表参道"; kana="おもてさんどう"; romaji="omotesandou"; shozoku="銀座線"};
...
(* 漢字の駅名をふたつと駅間リストを受け取ったら、2駅間の距離を返す関数 *)
...
let rec get_ekikan_kyori station1 station2 ekikan_list = match ekikan_list with
  ...
  | {kiten=kiten; shuten=shuten; keiyu=keiyu; kyori=kyori; jikan=jikan} :: rest ->
    if kiten = station1 && shuten = station2 then kyori
    else if kiten = station2 && shuten = station1 then kyori
    else get_ekikan_kyori station1 station2 rest

let test2 = get_ekikan_kyori "茗荷谷" "新大塚" global_ekikan_list = 1.2
let test3 = get_ekikan_kyori "茗荷谷" "渋谷" global_ekikan_list = infinity

(* ローマ字の駅名をふたつ受け取って、そのあいだの距離を調べる *)
(* kyori_wo_hyoji: string -> string -> string *)
let kyori_wo_hyoji station1 station2 =
  let station1_kanji = romaji_to_kanji station1 global_ekimei_list in let station2_kanji = romaji_to_kanji station2 global_ekimei_list in
    if station1_kanji = "" then station1 ^ "という駅は存在しません"
    else if station2_kanji = "" then station2 ^ "という駅は存在しません"
    else
      let ekikan_kyori = get_ekikan_kyori station1_kanji station2_kanji global_ekikan_list in
        if ekikan_kyori = infinity then station1_kanji ^ "駅と" ^ station2_kanji ^ "駅はつながっていません"
        else station1_kanji ^ "駅から" ^ station2_kanji ^ "駅までは" ^ string_of_float(ekikan_kyori) ^ "kmです"

let test4 = kyori_wo_hyoji "myogadani" "shinotsuka" = "茗荷谷駅から新大塚駅までは1.2kmです"
let test5 = kyori_wo_hyoji "myogadani" "shibuya" = "茗荷谷駅と渋谷駅はつながっていません"
let test6 = kyori_wo_hyoji "myogadani" "hoge" = "hogeという駅は存在しません"

(* ekimei_t型のリストを受け取って、eki_t型のリストを作る関数 *)
...
let make_eki_list ekimei =
  List.map (fun {kanji=kanji; kana=kana; romaji=romaji; shozoku=shozoku} -> {namae=kanji; saitan_kyori=infinity; temae_list=[]}) ekimei

let test7
= make_eki_list [{kanji="中野"; kana="なかの"; romaji="nakano"; shozoku="東西線"};
                {kanji="氷川台"; kana="ひかわだい"; romaji="hikawadai"; shozoku="有楽町線"}]
= [{namae="中野"; saitan_kyori=infinity; temae_list=[]};
   {namae="氷川台"; saitan_kyori=infinity; temae_list=[]}]

(* eki_t型のリストと起点を受け取ったら、起点のみ初期化する。 *)
(* shokika: eki_t list -> string -> eki_t list *)
(*
let rec shokika eki_t_origin kiten = match eki_t_origin with
  [] -> []
  | {namae=namae; saitan_kyori=station_kyori; temae_list=temae_list} :: rest ->
    if namae = kiten then {namae=namae; saitan_kyori=0.; temae_list=[kiten]} :: rest
    else {namae=namae; saitan_kyori=station_kyori; temae_list=temae_list} :: shokika rest kiten
*)
let shokika eki_t_origin kiten =
  List.map ((fun kiten {namae=namae; saitan_kyori=station_kyori; temae_list=temae_list}
        -> if namae = kiten then {namae=namae; saitan_kyori=0.; temae_list=[kiten]}
                            else {namae=namae; saitan_kyori=station_kyori; temae_list=temae_list}) kiten)
  eki_t_origin


let test8
= shokika [{namae="中野"; saitan_kyori=infinity; temae_list=[]};
           {namae="氷川台"; saitan_kyori=infinity; temae_list=[]}] "氷川台"
= [{namae="中野"; saitan_kyori=infinity; temae_list=[]};
   {namae="氷川台"; saitan_kyori=0.; temae_list=["氷川台"]}]


let make_initial_eki_list ekimei kiten =
  List.map ((fun kiten {kanji=kanji; kana=kana; romaji=romaji; shozoku=shozoku}
            ->  if kanji = kiten then {namae=kanji; saitan_kyori=0.; temae_list=[kiten]}
                                 else {namae=kanji; saitan_kyori=infinity; temae_list=[]}) kiten) ekimei

let test8_2
= make_initial_eki_list [{kanji="中野"; kana="なかの"; romaji="nakano"; shozoku="東西線"};
                         {kanji="氷川台"; kana="ひかわだい"; romaji="hikawadai"; shozoku="有楽町線"}] "氷川台"
= [{namae="中野"; saitan_kyori=infinity; temae_list=[]};
   {namae="氷川台"; saitan_kyori=0.; temae_list=["氷川台"]}]


(* 入力：あらかじめ昇順に並んでいるekimei_t型のリストとekimei_t *)
(* 出力：昇順に並んでいるekimei_t型のリスト *)
...
let rec insert lst ekimei = match lst with
 ...
 -> match ekimei with
    {kanji=kanji_; kana=kana_; romaji=romaji_; shozoku=shozoku_} ->
      if kana_ < kana then ekimei :: lst
                      else {kanji=kanji; kana=kana; romaji=romaji; shozoku=shozoku} :: insert rest ekimei

let input9_1 = [{kanji="青山一丁目"; kana="あおやまいっちょうめ"; romaji="aoyama-itchome"; shozoku="半蔵門線"};
                {kanji="表参道"; kana="おもてさんどう"; romaji="omotesandou"; shozoku="銀座線"};
                {kanji="渋谷"; kana="しぶや"; romaji="shibuya"; shozoku="銀座線"};
                {kanji="渋谷"; kana="しぶや"; romaji="shibuya"; shozoku="半蔵門線"}]

let input9_2 = {kanji="表参道"; kana="おもてさんどう"; romaji="omotesandou"; shozoku="半蔵門線"}

let output9 = [{kanji="青山一丁目"; kana="あおやまいっちょうめ"; romaji="aoyama-itchome"; shozoku="半蔵門線"};
                {kanji="表参道"; kana="おもてさんどう"; romaji="omotesandou"; shozoku="銀座線"};
                {kanji="表参道"; kana="おもてさんどう"; romaji="omotesandou"; shozoku="半蔵門線"};
                {kanji="渋谷"; kana="しぶや"; romaji="shibuya"; shozoku="銀座線"};
                {kanji="渋谷"; kana="しぶや"; romaji="shibuya"; shozoku="半蔵門線"}]

let test9 = insert input9_1 input9_2 = output9

(* 入力：ekimei_t型のリスト *)
(* 出力：昇順に整列したekimei_t型のリスト *)
...
let input10 = [{kanji="表参道"; kana="おもてさんどう"; romaji="omotesandou"; shozoku="銀座線"};
              {kanji="渋谷"; kana="しぶや"; romaji="shibuya"; shozoku="銀座線"};
              {kanji="渋谷"; kana="しぶや"; romaji="shibuya"; shozoku="半蔵門線"};
              {kanji="表参道"; kana="おもてさんどう"; romaji="omotesandou"; shozoku="半蔵門線"};
              {kanji="青山一丁目"; kana="あおやまいっちょうめ"; romaji="aoyama-itchome"; shozoku="半蔵門線"};]

let output10 = [{kanji="青山一丁目"; kana="あおやまいっちょうめ"; romaji="aoyama-itchome"; shozoku="半蔵門線"};
                {kanji="表参道"; kana="おもてさんどう"; romaji="omotesandou"; shozoku="半蔵門線"};
                {kanji="表参道"; kana="おもてさんどう"; romaji="omotesandou"; shozoku="銀座線"};
                {kanji="渋谷"; kana="しぶや"; romaji="shibuya"; shozoku="半蔵門線"};
                {kanji="渋谷"; kana="しぶや"; romaji="shibuya"; shozoku="銀座線"}]

let test10 = ins_sort input10 = output10

(* ソート済みのekimei_t型のリストを受け取ったら、それをひらがなの順に整列して駅の重複を取り除いたekimei_t型のリストを返す関数 *)
(* seiretsu_sorted: ekimei_t -> ekimei_t *)
let rec seiretsu_sorted ekimei_t_sorted = match ekimei_t_sorted with
  [] -> []
  | ({kanji=kanji1; kana=kana1; romaji=romaji1; shozoku=shozoku1} as first) :: rest
    -> match rest with
      [] -> [first]
      | {kanji=kanji2; kana=kana2; romaji=romaji2; shozoku=shozoku2} :: rest2
        -> if kana1 = kana2 then seiretsu_sorted rest
                            else first :: seiretsu_sorted rest

(* ekimei_t型のリストを受け取ったら、それをひらがなの順に整列して駅の重複を取り除いたekimei_t型のリストを返す関数 *)
(* seiretsu: ekimei_t -> ekimei_t *)
let seiretsu ekimei_t_origin =
  let ekimei_t_sorted = ins_sort ekimei_t_origin in
    seiretsu_sorted ekimei_t_sorted

let input11 = [{kanji="表参道"; kana="おもてさんどう"; romaji="omotesandou"; shozoku="銀座線"};
             {kanji="渋谷"; kana="しぶや"; romaji="shibuya"; shozoku="銀座線"};
             {kanji="渋谷"; kana="しぶや"; romaji="shibuya"; shozoku="半蔵門線"};
             {kanji="表参道"; kana="おもてさんどう"; romaji="omotesandou"; shozoku="半蔵門線"};
             {kanji="青山一丁目"; kana="あおやまいっちょうめ"; romaji="aoyama-itchome"; shozoku="半蔵門線"};]

let output11 = [{kanji="青山一丁目"; kana="あおやまいっちょうめ"; romaji="aoyama-itchome"; shozoku="半蔵門線"};
                {kanji="表参道"; kana="おもてさんどう"; romaji="omotesandou"; shozoku="銀座線"};
                {kanji="渋谷"; kana="しぶや"; romaji="shibuya"; shozoku="銀座線"};]


let test11 = seiretsu input11 = output11

(*
let p12 = {namae="渋谷"; saitan_kyori=1.5; temae_list=["hoge"; "渋谷"]}
let q12 = {namae="表参道"; saitan_kyori=infinity; temae_list=[]}
let output12 = {namae="表参道"; saitan_kyori=2.8; temae_list=["表参道"; "hoge"; "渋谷"]}

let test12 = koushin1 p12 q12 = output12

let p13 = {namae="渋谷"; saitan_kyori=1.5; temae_list=["hoge"; "渋谷"]}
let q13 = {namae="表参道"; saitan_kyori=1.9; temae_list=["hoge"; "fuga"]}
let output13 = {namae="表参道"; saitan_kyori=1.9; temae_list=["hoge"; "fuga"]}

let test13 = koushin1 p13 q13 = output13

let p14 = {namae="渋谷"; saitan_kyori=1.5; temae_list=["hoge"; "渋谷"]}
let q14 = {namae="東大前"; saitan_kyori=infinity; temae_list=[]}
let output14 = {namae="東大前"; saitan_kyori=infinity; temae_list=[]}
...
let p15 = {namae="渋谷"; saitan_kyori=1.5; temae_list=["hoge"; "渋谷"]}
let v15 = [{namae="表参道"; saitan_kyori=infinity; temae_list=[]};
